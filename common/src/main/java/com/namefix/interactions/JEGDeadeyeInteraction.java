...
public class JEGDeadeyeInteraction extends AbstractDeadeyeInteraction {
	public JEGDeadeyeInteraction(PlayerDeadeyeState state, Player player, ItemStack itemStack) {
	...
	public void postMark() {
		...
		if(JEGIntegration.getGunAmmo(itemStack) <= state.targets.size()) {
	...
	public boolean preShot() {
		...
		if(!JEGIntegration.isGun(held)) return false;
		if(!JEGIntegration.isGunReady(held, player)) return false;
		return true;
	}
	...
	public void shoot() {
		...
		JEGIntegration.fireGun(player.getMainHandItem(), player, targetData.target);
	}
