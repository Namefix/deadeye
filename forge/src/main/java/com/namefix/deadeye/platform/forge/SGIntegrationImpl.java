...
public class SGIntegrationImpl {
	private SGIntegrationImpl() {}

	public static boolean isLoaded() {
		ModList modList = ModList.get();
		return modList != null && modList.isLoaded("scguns");
	}
	...
	public static void refillAmmo(Player player, ItemStack item) {
		...
		if(reloadType == ReloadType.SINGLE_ITEM) {
			Item reloadItem = gun.getReloads().getReloadItem();
		...
		Item ammoItem = gun.getProjectile().getItem();
		if(ammoItem == null) return;

		ItemStack[] availableStacks = Gun.findAmmoStack(player, ammoItem);
		int availableAmmo = Math.max(ReloadTracker.ammoInInventory(availableStacks), 0);
